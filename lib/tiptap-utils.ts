...
export const handleImageUpload = async (
  ...
  try {
    ...
    // Import the media upload server action dynamically
    const { uploadMedia } = await import("@/app/(dashboard)/media/actions");

    // Report progress (simulated, as we can't track actual upload progress from server action)

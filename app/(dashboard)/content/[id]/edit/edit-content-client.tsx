...
import { useContent, useCategories, useTags } from "@/hooks/use-content-data";
...
export function EditContentClient({ contentId, authorName }: Props) {
  const { data: content, isLoading: contentLoading } = useContent(contentId);
  const { data: categories, isLoading: catLoading } = useCategories();
  const { data: tags, isLoading: tagsLoading } = useTags();

  if (contentLoading || catLoading || tagsLoading) {
    return (
